...
@dataclass
class KiMi25VLTransformerConfig(TransformerConfig):
    """Configuration for KiMi25-VL transformer with vision and language components."""
...
@dataclass
class KimiVLMultimodalProjectorConfig(TransformerConfig):
    """Kimi VL multimodal projector config"""

    input_size: int = 1152  # vision_config.hidden_size
    hidden_size: int = 2048  # text_config.hidden_size
    merge_kernel_size: Optional[List[int]] = field(default_factory=lambda: [2, 2])

    layernorm_epsilon: float = 1e-5  # text_config.rms_norm_eps

    num_layers: int = 1
    num_attention_heads: int = 1

    def configure_model(self) -> "KimiVLMultimodalProjector":
        """Get module"""
        return KimiVLMultimodalProjector(self)